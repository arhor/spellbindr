...
class LevelUpProgressionEngineTest {
    ...
    fun `rebuild should expose normal fixed gain when taking first level in a later class`() {
        ...
        val requirement = LevelUpProgressionEngine.rebuild(sheet, progression, plan, referenceData())
            .requirements
            .filterIsInstance<io.github.arhor.dnd.companion.domain.model.LevelUpRequirement.HitPoints>()
            .single()
    ...
    fun `rebuild should expose eligible feats and cap safe feat choices when prerequisites are met`() {
        ...
        val asi = preview.requirements
            .filterIsInstance<io.github.arhor.dnd.companion.domain.model.LevelUpRequirement.AbilityScoreImprovement>()
            .single()
        val featChoice = preview.requirements
            ...
            .single { it.category == io.github.arhor.dnd.companion.domain.model.LevelUpChoiceCategory.Feat }
    ...
    fun `rebuild should expose later subclass feature choices when a sticky subclass gains a feature`() {
        ...
        val champion = io.github.arhor.dnd.companion.domain.model.Subclass("champion", "Champion", emptyList(), subclassFlavor = "Martial", levels = listOf(ClassLevel("champion-3", 3, listOf(championFeature.id))))
        ...
        // Then
        assertThat(preview.requirements.filterIsInstance<io.github.arhor.dnd.companion.domain.model.LevelUpRequirement.ChoiceSelection>().map { it.id }).contains("champion-feature:choice")
    }
    ...
    fun `rebuild should reject a spell that is not on the selected class list when choosing a known spell`() {
        ...
        val bard = characterClass("bard", 8, emptyList()).copy(levels = (1..20).map { level ->
            ClassLevel("bard-$level", level, emptyList(), if (level == 1) io.github.arhor.dnd.companion.domain.model.LevelSpellcasting(2, 4, mapOf("1" to 2)) else null)
        })
    ...
    fun `rebuild should reject replacement when removed spell is not in reconstructed class spell state`() {
        // Given
        val bard = characterClass("bard", 8, emptyList()).copy(levels = (1..20).map { level ->
            ClassLevel("bard-$level", level, emptyList(), io.github.arhor.dnd.companion.domain.model.LevelSpellcasting(2, if (level == 1) 2 else 3, mapOf("1" to 2)))
        })
    ...
    fun `rebuild should expose per-class reason when multiclass prerequisite requires override`() {
        ...
        ).requirements.filterIsInstance<io.github.arhor.dnd.companion.domain.model.LevelUpRequirement.ClassSelection>().single()
