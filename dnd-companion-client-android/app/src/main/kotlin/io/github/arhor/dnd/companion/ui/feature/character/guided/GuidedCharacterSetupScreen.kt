...
import io.github.arhor.dnd.companion.ui.feature.character.guided.internal.StandardArray
import io.github.arhor.dnd.companion.ui.feature.character.guided.internal.firstIncompleteRequirement
import io.github.arhor.dnd.companion.ui.feature.character.guided.internal.pointBuyCost
import io.github.arhor.dnd.companion.ui.feature.character.guided.internal.requirementBlockingMessage
...
import io.github.arhor.dnd.companion.ui.feature.character.guided.model.GuidedValidationIssue
    ...
    if (selectedRace != null) {
        ...
        traitIds.mapNotNull { state.traitsById[it] }.forEach { trait ->
            trait.effects.orEmpty().forEach { effect ->
                if (effect is io.github.arhor.dnd.companion.domain.model.Effect.AddLanguagesEffect) {
...
internal fun computeAlreadySelectedProficiencyReasons(
    ...
    if (selectedRace != null) {
        ...
        traitIds.mapNotNull { state.traitsById[it] }.forEach { trait ->
            trait.effects.orEmpty().forEach { effect ->
                if (effect is io.github.arhor.dnd.companion.domain.model.Effect.AddProficienciesEffect) {
    ...
    return when (state.step) {
        ...
        GuidedStep.PROFICIENCIES_LANGUAGES ->
            state.choiceRequirements.firstOrNull {
                (it.category == GuidedChoiceCategory.PROFICIENCY ||
                    it.category == GuidedChoiceCategory.LANGUAGE) &&
                    !io.github.arhor.dnd.companion.ui.feature.character.guided.internal.isRequirementComplete(
                        it,
                        selection.choiceSelections,
                    )
            }?.let { requirementBlockingMessage(it, selection.choiceSelections) }
